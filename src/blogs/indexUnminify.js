...
window.addEventListener("load", (event) => {
  ...
  let section2HeaderTween = null;
  let section2CoverTween = null;
  let section2Active = false;

  if (section2) {
    const headerEl = section2.querySelector("header");
    if (headerEl) {
      section2HeaderTween = gsap.to(".section2", {
        scrollTrigger: {
          trigger: ".section2",
          start: "top top",
          end: "bottom top",
          ...
          pinSpacing: false,
          onEnter: () => {
            headerEl.style.borderBottomWidth = "0.1vmin";
          },
          onLeaveBack: () => {
            headerEl.style.borderBottomWidth = "0";
          },
        },
      });
    }
  ...
  function initSection2Triggers() {
    if (section2Active) return;
    if (!section2) return;
    if (window.innerWidth < 1500) return;

    const blogCover = section2.querySelector(".blogCoverImgContainer");
    const blogImg = blogCover ? blogCover.querySelector("img") : null;
    if (blogCover && blogImg) {
      section2CoverTween = gsap.to(".section2 .blogCoverImgContainer", {
        scrollTrigger: {
          trigger: ".section2",
          start: "top top",
          end: "bottom top",
          pin: ".section2 .blogCoverImgContainer > img",
          pinSpacing: false,
        },
      });
    }
  ...
  function destroySection2Triggers() {
    if (section2CoverTween) {
      try {
        if (section2CoverTween.scrollTrigger) section2CoverTween.scrollTrigger.kill();
        section2CoverTween.kill();
      } catch (e) {}
      section2CoverTween = null;
    }
    if (section2) {
    }
